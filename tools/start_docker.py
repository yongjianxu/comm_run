...
class mycontainer:
    def __init__(self):
        ...
        #cmd = "docker run --name mytest --cap-add=SYS_NICE --cap-add=IPC_LOCK --cap-add=SYS_ADMIN --gpus all --ulimit memlock=-1 --ulimit stack=2000000000 --init -ti -p 12345:12345 -p 2379:2379 --shm-size 32g --network=host -v /usr/src:/usr/src -v /lib/modules:/lib/modules --device=/dev/infiniband/ --device=/dev/gdrdrv " + " " + image + " " + "bash"
        print(cmd) 
        os.system(cmd) 
